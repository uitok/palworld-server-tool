...
func getServerTaskStatus(c *gin.Context) {
	c.JSON(http.StatusOK, task.StatusSnapshot())
}
...
func publishBroadcast(c *gin.Context) {
	...
	writeOperationSuccess(c, "broadcast", "", "manual", "broadcast completed", time.Since(startedAt).Milliseconds(), gin.H{"message": req.Message})
}
...
func shutdownServer(c *gin.Context) {
	...
	writeOperationSuccess(c, "shutdown", "", "manual", "shutdown scheduled", time.Since(startedAt).Milliseconds(), gin.H{"seconds": req.Seconds, "message": req.Message})
}
